...
public class AppFilter extends BasicHttpAuthenticationFilter {
    ...
    @Autowired
    public void AppFilter(RedisService redisService) {
        AppFilter.redisService = redisService;
    }
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        synchronized (this) {
            ...
            if (redisService.exists(Constant.PREFIX_SHIRO_APP_REFRESH_TOKEN + account)) {
                ...
                if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                    ...
                    String appRefreshTokenExpireTime = PropertiesUtil.getProperty("appRefreshTokenExpireTime");
                    // 设置RefreshToken中的时间戳为当前最新时间戳，且刷新过期时间重新为30分钟过期(配置文件可配置refreshTokenExpireTime属性)
                    redisService.set(Constant.PREFIX_SHIRO_APP_REFRESH_TOKEN + account, currentTimeMillis, Long.parseLong(appRefreshTokenExpireTime));
                    // 将当前用户信息存入redis，方便其他接口获取
                    Object currentUser = redisService.get(Constant.PREFIX_APP_CURRENT_USER + account);
                    redisService.set(Constant.PREFIX_APP_CURRENT_USER + account, currentUser, Long.parseLong(appRefreshTokenExpireTime));
                    // 刷新AccessToken，设置时间戳为当前最新时间戳
                    token = JwtUtil.sign(account, currentTimeMillis, Constant.LOGIN_TYPE_APP);
                    ...
                    // 保存当前旧的Token
                    redisService.set(Constant.PREFIX_SHIRO_APP_REFRESH_TOKEN_TRANSITION + account, this.getAuthzHeader(request), Long.parseLong(refreshTokenTransitionExpireTime));
    ...
    private void verifyLoginType(ServletRequest request, ServletResponse response) {
        ...
        if (!Constant.LOGIN_TYPE_APP.equals(loginType)) {
