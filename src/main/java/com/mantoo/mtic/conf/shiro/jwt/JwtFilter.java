...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    @Autowired
    public void JwtFilter(RedisService redisService) {
        JwtFilter.redisService = redisService;
    }
        ...
        synchronized (this) {
            ...
            if (redisService.exists(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
                ...
                if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                    ...
                    Object currentUser = redisService.get(Constant.PREFIX_CURRENT_USER + account);
                    redisService.set(Constant.PREFIX_CURRENT_USER + account, currentUser, Long.parseLong(refreshTokenExpireTime));
                    // 刷新AccessToken，设置时间戳为当前最新时间戳
                    token = JwtUtil.sign(account, currentTimeMillis, Constant.LOGIN_TYPE_WEB);
                    ...
                    // 保存当前旧的Token
                    redisService.set(Constant.PREFIX_SHIRO_REFRESH_TOKEN_TRANSITION + account, this.getAuthzHeader(request), Long.parseLong(refreshTokenTransitionExpireTime));
    ...
    private void verifyLoginType(ServletRequest request, ServletResponse response) {
        ...
        if (!Constant.LOGIN_TYPE_WEB.equals(loginType)) {
