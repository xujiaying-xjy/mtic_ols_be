...
/**
 * @ClassName: MiniAppFilter
 * @Description: 小程序过滤器
 * @Author: renjt
 * @Date: 2020-06-22 15:46
 */
@Component
public class MiniAppFilter extends BasicHttpAuthenticationFilter {

    private static final Logger logger = LoggerFactory.getLogger(MiniAppFilter.class);

    private static RedisService redisService;

    @Autowired
    public void MiniAppFilter(RedisService redisService) {
        MiniAppFilter.redisService = redisService;
    }
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        synchronized (this) {
            ...
            if (redisService.exists(Constant.PREFIX_SHIRO_MINIAPP_REFRESH_TOKEN + account)) {
                ...
                if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                    ...
                    String miniappRefreshTokenExpireTime = PropertiesUtil.getProperty("miniappRefreshTokenExpireTime");
                    // 设置RefreshToken中的时间戳为当前最新时间戳，且刷新过期时间重新为30分钟过期(配置文件可配置refreshTokenExpireTime属性)
                    redisService.set(Constant.PREFIX_SHIRO_MINIAPP_REFRESH_TOKEN + account, currentTimeMillis, Long.parseLong(miniappRefreshTokenExpireTime));
                    // 将当前用户信息存入redis，方便其他接口获取
                    Object currentUser = redisService.get(Constant.PREFIX_MINIAPP_CURRENT_USER + account);
                    redisService.set(Constant.PREFIX_MINIAPP_CURRENT_USER + account, currentUser, Long.parseLong(miniappRefreshTokenExpireTime));
                    // 刷新AccessToken，设置时间戳为当前最新时间戳
                    token = JwtUtil.sign(account, currentTimeMillis, Constant.LOGIN_TYPE_MINIAPP);
                    ...
                    // 保存当前旧的Token
                    redisService.set(Constant.PREFIX_SHIRO_MINIAPP_REFRESH_TOKEN_TRANSITION + account, this.getAuthzHeader(request), Long.parseLong(refreshTokenTransitionExpireTime));
    ...
    private void verifyLoginType(ServletRequest request, ServletResponse response) {
        ...
        if (!Constant.LOGIN_TYPE_MINIAPP.equals(loginType)) {
