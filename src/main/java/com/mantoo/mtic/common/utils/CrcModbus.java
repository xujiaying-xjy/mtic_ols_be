...
public class CrcModbus {
	...
	public static boolean CrcValid(byte[] bytes) {
		byte[] validCode = new byte[2];
		copyBytes(bytes, bytes.length - 2, validCode, 0, 2);
		int crc1 = ByteUtil.CRC16(bytes, 0, bytes.length - 2);
		int crc2 = ByteUtil.byte2Int(validCode, false);
		return crc1 == crc2;
	}
	
	public static void copyBytes(byte[] origin, int origin_offset, byte[] target, int target_offset, int limit) {
		for(int i = 0; i < limit; i++) {
			target[target_offset + i] = origin[origin_offset + i];
		}
	}
}
