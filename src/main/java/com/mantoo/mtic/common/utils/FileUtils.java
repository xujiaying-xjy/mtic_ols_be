...
public class FileUtils {
    ...
    public static String generateFileName(MultipartFile file){
        if (file != null) {
            ...
            String fileName = Objects.requireNonNull(file.getOriginalFilename()).substring(0, file.getOriginalFilename().lastIndexOf(".")) +
                    "-" + time + file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf("."));
            return fileName;
        } else {
            return null;
        }
     ...
     * @return java.lang.String
     * @Description 上传附件工具类
     * @Param [file, winPath, linuxPath, fileName]
     * @Author renjt
     * @Date 2019-11-29 14:39
     */
    public static String mticSaveFile(MultipartFile file, String winPath, String linuxPath, String fileName) {
        String line = File.separator; // 当前系统下的斜杠
        String path = "";
        if ("\\".equals(line)) {
            path = winPath;
        } else if ("/".equals(line)) {
        ...
        try {
            ...
            FileUtils.writeBytes2File(file.getBytes(), newFile);
            return path;
        } catch (IOException e) {
            e.printStackTrace();
            throw new MticException(e.getMessage(), ErrorInfo.SERVICE_ERROR.getCode());
        }
    }
     ...
     */
    public static void mticDownloadFile(String filePath, HttpServletResponse response) {
        //验证文件是否存在
        File file = new File(filePath);
        if (!file.exists()) {
            throw new MticException("源文件不存在", ErrorInfo.NOT_EXIST.getCode());
        }
        ...
        try {
            ...
            e.printStackTrace();
            throw new MticException("文件下载失败", ErrorInfo.NOT_EXIST.getCode());
        } finally {
            //关闭输入输出流
            if (toClient != null) {
                try {
