...
public class AesCipherUtil {
    ...
    public static String enCrypto(String str) {
        try {
            ...
            throw new MticException("getInstance()方法异常:" + e.getMessage(), ErrorInfo.SERVICE_ERROR.getCode());
        } catch (UnsupportedEncodingException e) {
            logger.error("Base64加密异常:" + e.getMessage());
            throw new MticException("Base64加密异常:" + e.getMessage(), ErrorInfo.SERVICE_ERROR.getCode());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:" + e.getMessage());
            throw new MticException("初始化Cipher对象异常:" + e.getMessage(), ErrorInfo.SERVICE_ERROR.getCode());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("加密异常，密钥有误:" + e.getMessage());
            throw new MticException("加密异常，密钥有误:" + e.getMessage(), ErrorInfo.SERVICE_ERROR.getCode());
        }
    }
    ...
    public static String deCrypto(String str) {
        try {
            ...
            throw new MticException("Base64解密异常:" + e.getMessage(), ErrorInfo.SERVICE_ERROR.getCode());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:" + e.getMessage());
            throw new MticException("初始化Cipher对象异常:" + e.getMessage(), ErrorInfo.SERVICE_ERROR.getCode());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("解密异常，密钥有误:" + e.getMessage());
            throw new MticException("解密异常，密钥有误:" + e.getMessage(), ErrorInfo.SERVICE_ERROR.getCode());
        }
    }
}
